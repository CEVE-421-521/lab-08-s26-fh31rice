...
println("results_vec length = ", length(results_vec))
println("total_costs length = ", length(total_costs))
```
...
println("$(n_fail) of $(n_scenarios) total runs exceed the budget ($(round(100 * n_fail / n_scenarios; digits=1))%)")
```
...
*In 2–3 sentences, how might a different failure definition change what PRIM finds?* Ans: For the baseline case, failure was 684 out of 1000 (68.4%), budget was 672 M, for case 1: u =150 cm, total failues were 261 (26.1%) with budget 1285M, for case 2, with alpha=0.080, total failure 713, which is 71.3%, total budget 649M, for case 3 with eta= 1.5 cm/year, 220 out of 1000 failed, budget = 1412 M.

So, the takeaway message is that, for case 1, heightening dike, doubled the budget from the abse case, and fewer runs got to cross threshold and also the failures reduced from 684 to 261. For, case 2, budget decreased compared to base case and failure increased. For the last case, I have increased sea level rise, and total budget increased by more than 50% compared to base case. So based on what we call a failure, the number of failure case changes.
:::
...
benchmark_policy = EijgenraamPolicy(; t_heighten=0.0, u_heighten=150.0)
ref_result = explore(config, [ref_scenario], [benchmark_policy])
cost_budget = Array(ref_result.total_cost)[1, 1]
println("Cost budget: $(round(cost_budget; digits=0)) M€  (benchmark policy under reference scenario)")
...
ref_scenario = EijgenraamScenario(;
    P0=P0_fixed,
    alpha=0.080,   # midpoint of [0.025, 0.075]
    eta=0.8,      # midpoint of [0.4, 1.2]
    V0=V0_fixed,
    delta=delta_fixed,
)
...
good_idx = findfirst(b -> b.density > 0.7 && b.coverage > 0.5, trajectory)
if good_idx === nothing
...
Ans: Total acost is unacceptably high when t_heighten ≤ 38.5 AND u_heighten ≥ 185. Box 28, is only for policy levers, no climate aprameters. This means policy decision drives high cost in the model.
:::
...
let
    ...
    for (j, (name, pos)) in enumerate(zip(param_names, positions))
        ...
        hist!(ax, X[pass_mask, j];
            bins = 20,
            normalization = :pdf,
            color = (:steelblue, 0.6)
        )

        hist!(ax, X[fail_mask, j];
            bins = 20,
            normalization = :pdf,
            color = (:tomato, 0.6)
        )
    ...
    Legend(
        fig[2, 1:4],
        [PolyElement(; color = (:steelblue, 0.6)),
         PolyElement(; color = (:tomato, 0.6))],
...
Ans: For comparison, I choose u_heightening and t_heightening. The plot shows that when u_heighten is high, most points are failures, and when it is low, most points are passing (if we take vertical slices). So u_heighten is an important variable. This matches PRIM. The variable t_heighten is less important because both pass and fail points are mixed at all values (if we take horizontal slice).
...
let
    ...
    ax = Axis(
        fig[1, 1];
        xlabel = "u_heighten (cm)",
        ylabel = "t_heighten (years)",
        title = "Pass vs Fail Comparison"
    )

    scatter!(
        ax,
        X[pass_mask, 4], X[pass_mask, 3];
